...
- Update oracle with `unique_id`, `pcr_values` and `public_key`'s. You can get `unique_id` and `public_key` by requesting `/info` endpoint of the [SGX Oracle Notarization backend](https://sgx.aleooracle.xyz/info) and `pcr_values` with `public_key` from `/info` of the [Nitro Oracle Notarization backend](https://nitro.aleooracle.xyz/info)
```
...
You can find this program deployed in the mainnet by visiting the [official Aleo Explorer](https://explorer.aleo.provable.com/program/official_oracle.aleo) or [Aleo123 explorer](https://mainnet.aleo123.io/programDetail/official_oracle.aleo).

### Testnet Oracle program

**`official_oracle_v2.aleo`**

You can find this program deployed in the testnet by visiting the [official Aleo Explorer](https://testnet.explorer.provable.com/program/official_oracle_v2.aleo) or [Aleo123 explorer](https://testnet.aleo123.io/programDetail/official_oracle_v2.aleo).

!!! tip

    You may have seen and/or used `official_oracle.aleo` program in testnet. That program is not supported anymore, and you
    should use `official_oracle_v2.aleo` instead. See [the migration guide](#migration-guide) for more information.
...
`sgx_attested_data` stores the data acquired by an SGX notarizer with a timestamp from a specific request. You can always access the stored data using your [Request Hash](./understanding_response.md/#about-request-hash) as a key or [Timestamped Request Hash](./understanding_response.md#timestamped-request-hash) to get data the that was produced at a specific timestamp.

#### nitro_attested_data

!!! example "nitro_attested_data"
...
`nitro_attested_data` stores the data acquired by a Nitro notarizer with a timestamp from a specific request. You can always access the stored data using your [Request Hash](./understanding_response.md/#about-request-hash) as a key or [Timestamped Request Hash](./understanding_response.md#timestamped-request-hash) to get the data that was produced at a specific timestamp.
...
This function is used to update the Oracle program with the new attested data from a Nitro notarizer. It takes the `report_data` that was included in the report, the `report` itself, the Schnorr `signature` of the provided report, the `public_key` to verify this signature, the `data_position` to extract the data hash from the report, `pcr_0_position`, `pcr_1_position` and `pcr_2_position` to extract the `PCR values` to verify that a report was produced using a specific version of the source code.
As a first step oracle extracts the data hash from the report and then verifies the report itself (see [`verify_nitro_report`](#verify_nitro_report)) and its signature.
Then it creates the `request_hash` (see [`get_request_hash`](#get_request_hash)). This hash is going to be used as the key to store the provided `attested_data` as the latest data produced by the attestation request. Then it computes a timestamped request hash using
the [`TimestampedHash`](#timestampedhash) helper type and the Poseidon8 hash. That hash is going to be used as a key to store the provided `attested_data` as a historical data
that is not going to be overwritten. You can read more about how the `Request Hash` and the `Timestamped Request Hash` works [here](./understanding_response.md/#about-request-hash).
Then it extracts the `pcr_values` from the report to verify them later.
...
You can read more about data extraction from Nitro reports [here](./understanding_response.md#nitro).

#### select_chunk

!!! example "select_chunk"
...
In case you want to use the data that was attested at a specific timestamp you can use [Timestamped Request Hash](./understanding_response.md#timestamped-request-hash) as a key to attested data mappings. It contains the [Request Hash](./understanding_response.md/#about-request-hashes) that was used to get the data as well as the timestamp of when the this data was notarized.

!!! example "Using timestamped_request_hash to query the Aleo Oracle"
...
- the mainnet network - [official_oracle.aleo](https://mainnet.aleo123.io/programDetail/official_oracle.aleo);
- and the testnet network - [official_oracle_v2.aleo](https://testnet.aleo123.io/programDetail/official_oracle_v2.aleo).

**They are the same program**.

For comparison, here is the old version of the Oracle program in the testnet network - [official_oracle.aleo (testnet)](https://testnet.aleo123.io/programDetail/official_oracle.aleo). This program is not supported anymore.
...
2. Change reads of the `attested_data` mapping to `sgx_attested_data`, which now returns an [`AttestedData`](#attesteddata) struct instead of the attested data `u128` value. Use `data` property to access the data;
...
See more in [`set_data_sgx`](#set_data_sgx) and the [Timestamped Request Hash](./understanding_response.md#timestamped-request-hash).
