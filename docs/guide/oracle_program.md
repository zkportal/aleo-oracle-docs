...
You can also run the Oracle Notarization backend yourself using reproducible builds and see the generated unique ID. It should mach the `unique_id` provided by other Oracle Notarization backends if they run the same code.
...
- If key was not pressent or not allowed then it becomes allowed
